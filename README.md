...
> 動画選択をスマホからも行えるよう、デッキ動画には音声を載せている（出力ウィンドウを前面にするとコントローラ窓が裏に回り、音声のない動画はブラウザに省電力停止される。これを防ぐため）。スピーカーはOS側でミュートする運用を想定。詳細は [ADR-0011](docs/adr/0011-keep-deck-videos-audible-to-avoid-background-freeze.md)。
