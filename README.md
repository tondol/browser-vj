...
ブラウザ上で動作するシンプルな2デッキVJツール。依存最小限のシンプルな構成でローカルのmp4を出力・クロスフェードできる。
...
```sh
npm install
npm run dev      # 開発サーバ + スマホリモコン中継サーバを同時起動（http://localhost:5173）
npm run build    # 型チェック + dist/ へビルド
npm run preview  # ビルド結果の確認
npm test         # ヘッドレス Chrome での E2E スモークテスト（test/ 参照）
```

基本機能に限れば、ビルド成果物は静的ファイルのみなので、任意の静的ホスティングで配信できる（同一オリジンで `index.html` と `output.html` を配信すること）。
...
`npm run dev` 実行で中継サーバ（既定ポート 8787、`PORT` で変更可）も一緒に起動する。中継サーバだけ起動したいときは `npm run dev:bridge`（フロントだけは `npm run dev:vite`）。

1. `npm run dev` で起動した状態で、コントローラのヘルプ（「？」ボタン）を開く
2. PCとスマホを同じWi-Fiに繋いだ状態で、「スマホでリモコン」欄のQRをスマホで読み取る
3. スマホのリモコン画面で再生/停止・フェーダー（スライダー操作とA/Bへの一発切替）・ナッジ・ライブラリからのロードができる
